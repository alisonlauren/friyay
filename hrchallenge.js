...
function twoNumberSum(array, targetSum) {
	...
	for (let i = 0; i < array.length - 1; i++) {
        //setting varialbe for wherever the pointer is at
        //at the given time
...
//sequence = [1, 6, -1, 10]
//output = true
//the reason its true is because the 1, 6, -1, and 10 are in the same order
//as the array, regardless of the fact that is has fewer values.
//its really just an order question

function isValidSubsequence(array, sequence) {
    //setting up my pointers, one for the array and one the sequence
	let arrIdx = 0;
    let seqIdx = 0;
    // while arrIdx pointer never exceeds the array length
    //and same for the seqIdx and sequence length
	while (arrIdx < array.length && seqIdx < sequence.length) {
        // if the arrIdx and seqIdx ever match as the same number,
        //then move on in the list in the array and the sequence
		if (array[arrIdx] === sequence[seqIdx]) seqIdx++;
		arrIdx++;
    }
    //this will basically return true or false based on the 
    //condition that seqIdx is absolutely equal to sequence.length
